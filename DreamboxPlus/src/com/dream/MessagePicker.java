...
public class MessagePicker extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setTimeout(b.getString("Timeout"));

		setMessageType(b.getLong("MessageType", 1));
	...
	private void ShowStartMessage() {
		...
		if (isDeleteMessageSelected() || isSaveSelected()) {
			...
			ToastMessage.Show("Select category to be deleted!!!",
			...
			if (resultCode == RESULT_OK) {
				...
				if (isDeleteMessageSelected()) {
					...
					for (; messageIteractor.hasNext();) {
						...
						if (message.Category.equals(category)
								&& message.Message != null
								&& message.Message.equals(messageSelected)) {
							messageIteractor.remove();
							ToastMessage.Show("Message deleted",
									getApplicationContext());
							break;
						}
					}
					...
					Message message = GetMessage(category, messageSelected);
					intent.putExtra("MessageSelected", messageSelected);

					if (message != null) {
						intent.putExtra("Timeout", message.Timeout);
						intent.putExtra("MessageType", message.Message);
					} else {
	...
	private Message GetMessage(String category, String message) {
		Iterator<Message> messageIteractor = getMessageCollection().iterator();

		for (; messageIteractor.hasNext();) {
			Message mess = messageIteractor.next();
			if (mess.Category.equals(category) && mess.Message != null
					&& mess.Message.equals(message)) {
				return mess;
			}
		}
		return null;
	...
	private void LoadMessageCollection() {
		...
		MessageCollection messageCollection = (MessageCollection) Filer.Load(
				fis, getApplicationContext());
	...
	private void SaveMessageCollection(boolean showMessage) {
		...
		try {
			...
			e.printStackTrace();
			ToastMessage.Show(
					"Failed to open file for saving " + e.getMessage(),
					getApplicationContext());
			return;
		}

		boolean result = Filer.Save(getMessageCollection(), fos,
				getApplicationContext());
	...
	private void CreateDefaultMessages() {
		...
		messageCollection.messages.add(CreateDefaultMessage("For Partner",
				"Put the kettle on love"));
		messageCollection.messages.add(CreateDefaultMessage("For Partner",
				"It's my turn for the remote control"));
		messageCollection.messages.add(CreateDefaultMessage("For Partner",
				"What is on the other side?"));
		messageCollection.messages.add(CreateDefaultMessage("System Error",
				"This system will meltdown"));
		messageCollection.messages.add(CreateDefaultMessage("For Kids",
				"It's time for bed"));
		messageCollection.messages.add(CreateDefaultMessage("For Kids",
	...
	private String[] GetCategories() {
		...
		String[] categoriesListArray = (String[]) categoriesList
				.toArray(new String[categoriesList.size()]);
	...
	private String[] GetMessages(String category) {
		...
		String[] messagesListArray = (String[]) messagesList
				.toArray(new String[messagesList.size()]);
	...
	private long getMessageType() {
		return messageType;
	}

	private void setMessageType(long messageType) {
